...
pictures = pd.read_csv('./static/csv/pictures.csv')
scores = pd.read_csv('./static/csv/new_pics_new_model_predictions.csv')
...
@app.route('/get_score', methods=['POST_ID', 'POST'])
def get_score():
	if request.method == 'POST':
		...
		r = request.get_json(force=True)
		#print(df_inner[df_inner.folder == r['folder']][df_inner.segment == r['segment']].score.iloc[0], flush = True)
		line = df_inner[df_inner.folder == r['folder']][df_inner.segment == r['segment']]
		score = line.score.iloc[0]
		picture = line.jpg.iloc[0]
		message = {"score" : round(score,2), "picture": picture}
